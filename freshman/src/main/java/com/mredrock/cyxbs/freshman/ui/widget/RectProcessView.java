...
public class RectProcessView extends View {
    ...
    private void setData(int width, int height) {
        ...
        float everyHeight = height / 6;
        dashLocation = new float[6];
        for (int i = 0; i < 6; i++) {
    ...
    private void drawCoordinate(Canvas canvas) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            canvas.translate(columnarLocation[2 * (i + 1)] - columnarLocation[0] / 2, dashLocation[5] + height / 2);
            layout.draw(canvas);
            canvas.translate(-(columnarLocation[2 * (i + 1)] - columnarLocation[0] / 2), -(dashLocation[5] + height / 2));
        }
        for (int i = 0; i < num; i++) {
            ...
            rectF.bottom = dashLocation[5];
            rectF.top = dashLocation[5] - dp2px(5);
    ...
    private void drawRects(Canvas canvas) {
        for (int i = 0; i < num; i++) {
            float pro = current[i] / max * dashLocation[5];
            rectF.left = columnarLocation[2 * (i + 1)];
            rectF.right = columnarLocation[2 * (i + 1) + 1];
            rectF.bottom = dashLocation[5];
            rectF.top = dashLocation[5] - pro;
    ...
    public void setAnim(float[] processes, float time) {
